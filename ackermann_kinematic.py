...
# Run the simulation
for k in range(1, N):
    x[:, k] = integration.rk_four(vehicle.f,
                                  x[:, k-1], u[:, k-1], T,
                                  ELL_W)
    phi_L[k] = np.arctan(2*ELL_W*np.tan(x[3, k]) /
                         (2*ELL_W-ELL_T*np.tan(x[3, k])))
    phi_R[k] = np.arctan(2*ELL_W*np.tan(x[3, k]) /
                         (2*ELL_W+ELL_T*np.tan(x[3, k])))
    u[0, k] = 5.0
...
ani = vehicle.animate(x, T, phi_L, phi_R, ELL_W, ELL_T, True,
